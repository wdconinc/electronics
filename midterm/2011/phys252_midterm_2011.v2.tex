% -*- latex -*-
% FILE: "/home/evmik/jobs/wm/2011_spring_Analog_Electronics_252/midterm_week_5/phys252_midterm_2010.v2.tex"
% LAST MODIFICATION: "Wed, 23 Feb 2011 13:36:59 -0500 (evmik)"
...
\includegraphics[width=0.25\textwidth]{./schematics/rc_high_pass}
	\begin{parts}
		...
		\part[5]
			...
			signal is $V_{in}=2$~V, $R=1$~kOhm, $C=5$~nF, and $\omega = 1/(R
			C)$.
			\vskip 3in
		\part[5]
...
\includegraphics[width=0.25\textwidth]{./schematics/loaded_voltage_divider}
	\begin{parts}
		...
		\part[10]
			...
			is the value of the current passing through the $R_1$, if $V_{in}=5$~V
			DC?
	\end{parts}
\pagebreak
...
Consider the circuit shown  below.\\
	...
	\begin{parts}
		\part[10]
			...
			\vskip 2in
		\part[10]
			If $R_1=20$~kOhm, $R_2=10$~kOhm, $R_3=2$~kOhm, $E_1=20$~V, and
			$E_2=10$~V.
			Find the currents $I_1, I_2$ and $I_3$.
			\vskip 3in
		\part[5]
			If resistors remain the same as before but $E_1$ and $E_2$ are
			decreased by factor of 2. What is the new value of $I_2$?
	\end{parts}
}
...
noise couples at frequency $f_n=10$~kHz with amplitude $V_n=10$~V.
	...
	\begin{parts}
		...
		\part[5]
			...
			through the lab and find a low-pass filter with $f_{3dB} = 0.1$~kHz.
		...
		\part[10]
			...
			$f_{3dB}$ of the low-pass filter at this conditions? Prove it. 
			\vskip 3in
		\part[5]
